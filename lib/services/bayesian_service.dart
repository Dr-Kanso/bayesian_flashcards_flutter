...
class BayesianService {
  static List<double> bayesianPosterior(Card card,
      {double priorAlpha = 1.0, double priorBeta = 1.0}) {
  ...
  double adaptiveDecay(Card card, User userProfile,
      {double? baseDecay, int historyWindow = 5}) {
  ...
  Map<String, dynamic> sampleNextReview(Card card, User userProfile,
  ...
  static double _gammaDistribution(double shape, Random random) {
    ...
    // In production, use a proper statistical library
    if (shape < 1) {
      return _gammaDistribution(shape + 1, random) *
          pow(random.nextDouble(), 1 / shape);
    }
